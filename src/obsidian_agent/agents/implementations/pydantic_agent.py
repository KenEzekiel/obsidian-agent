...
class PydanticAgentSystem(AgentSystem):
    ...
    def _suggest_links(
        ...
        console.print(
            "\n[bold blue]Pydantic[/] 🔗 Suggesting links for note:",
            self._escape_path(current_note_id),
        )
            ...
            if suggested_links:
                for link in suggested_links:
                    try:
                        ...
                        console.print(
                            f"  From: [cyan]{self._escape_path(current_note_id)}[/]"
                        )
                        console.print(
    ...
    def process_note(self, note_id: str, content: str, vault_path: str) -> AgentState:
        ...
        state_table.add_row(
            "Content Length", str(len(state.context.get("current_note_content", "")))
        )
        ...
        try:
            ...
            similar_notes = self.similarity_tool.func(
                note_id=note_id, top_n=5, exclude_ids=[note_id]
            )
            ...
            if (
                ...
                or state.context.get("topics")
                or state.context.get("note_categories")
            ):
