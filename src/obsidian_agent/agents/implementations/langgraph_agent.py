...
class LangGraphAgentSystem(AgentSystem):
    ...
    def _categorize_note_node(self, state: LangGraphState) -> LangGraphState:
        ...
        console.print(
            "\n[bold blue]LangGraph[/] 📑 Categorizing note:",
    ...
    def _analyze_content_node(self, state: LangGraphState) -> LangGraphState:
        ...
        console.print(
            "\n[bold blue]LangGraph[/] 📝 Analyzing content for note:",
            state["state"].current_note_id,
        )
        ...
        try:
            ...
            console.print(
                "[green]✓[/] Successfully analyzed content and identified topics"
            )
    ...
    def _find_similar_node(self, state: LangGraphState) -> LangGraphState:
        ...
        try:
            ...
            similar_notes = self.similarity_tool.func(
                note_id=current_note_id, top_n=5, exclude_ids=[current_note_id]
            )
    ...
    def _generate_links_node(self, state: LangGraphState) -> LangGraphState:
        ...
        console.print(
            "\n[bold blue]LangGraph[/] 🔗 Generating links for note:",
            self._escape_path(state["state"].current_note_id),
        )
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """You are an expert at suggesting meaningful connections between notes. You answer in this JSON FORMAT: 
            {{
                "links": [
                    {{
                        "note_id": "note_id",
            ...
            """,
                ...
                (
                    "human",
                    """Given the current note and similar notes, suggest meaningful links.
        ...
        console.print(
            Panel(response.content, title="🤖 LLM Response", border_style="blue")
        )
            ...
            if suggested_links:
                for link in suggested_links:
                    try:
                        ...
                        console.print(
                            f"  From: [cyan]{self._escape_path(state['state'].current_note_id)}[/]"
                        )
                        console.print(
                            f"  To: [cyan]{self._escape_path(link['note_id'])}[/]"
                        )
                        console.print(
                            f"  Confidence: [green]{link['confidence_score']}[/]"
                        )

                        if float(link["confidence_score"]) >= 0.5:
                            console.print(
                                "[yellow]ℹ️[/] Link confidence meets threshold (>= 0.5), proceeding..."
                            )
                        else:
                            console.print(
                                "[yellow]ℹ️[/] Link confidence below threshold (< 0.5), skipping..."
                            )
                            continue
                        self.add_link(
                            state["state"],
    ...
    def _update_clusters_node(self, state: LangGraphState) -> LangGraphState:
        ...
        for topic in topics:
            ...
            if topic:
                self.add_to_cluster(
                    state["state"], topic, state["state"].current_note_id
                )
    ...
    def _write_suggestions_to_note(self, state: AgentState, vault) -> None:
        ...
        console.print(
            "\n[bold blue]LangGraph[/] 📝 Writing suggestions to note:",
            ...
            table.add_row(
                ...
                str(
                    len(
                        [
                            t
                            for t, notes in state.topic_clusters.items()
                            if state.current_note_id in notes
                        ]
                    )
                ),
    ...
    def process_note(self, note_id: str, content: str, vault_path: str) -> AgentState:
        ...
        try:
            ...
            existing_note = self.vectorstore.collection.get(
                ids=[note_id], include=["embeddings", "metadatas", "documents"]
            )
            ...
            table.add_row(
                "Has embeddings",
                "Yes" if existing_note["embeddings"] is not None else "No",
            )
            if existing_note["embeddings"] is not None:
                table.add_row(
                    "Embedding length", str(len(existing_note["embeddings"][0]))
                )

            console.print(table)

            if (
                existing_note["embeddings"] is None
                or len(existing_note["embeddings"]) == 0
            ):
                ...
                verify_table.add_row(
                    "Has embeddings",
                    "Yes" if check_note["embeddings"] is not None else "No",
                )
                if check_note["embeddings"] is not None:
                    verify_table.add_row(
                        "Embedding length", str(len(check_note["embeddings"][0]))
                    )
                ...
                if not check_note["embeddings"] or len(check_note["embeddings"]) == 0:
                    console.print(
                        "[red]❌[/] Warning: Note was added but embeddings are still missing"
                    )
            else:
            ...
            return AgentState(
                current_note_id=note_id, error=f"Vector store error: {str(e)}"
            )
        ...
        state_table.add_row(
            "Content Length",
            str(len(initial_state.context.get("current_note_content", ""))),
        )

        console.print(state_table)

        console.print("\n[bold]🚀 Running workflow...[/]")
        result = self.graph.invoke({"state": initial_state, "messages": []})

        # Write suggestions and tags using VaultWriter
        if (
            result["state"].suggested_links
            or result["state"].context.get("topics")
            or result["state"].context.get("note_categories")
        ):
